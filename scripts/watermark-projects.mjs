...
/**
 * Public brand shown on client work. Matches the domain and the studio's
 * outward-facing name; `siteConfig.name` ("AB Digital Solutions") remains the
 * legal/organisation name used in schema.org, titles and the footer.
 */
...
export function buildWatermarkSvg(width, height) {
  ...
  <text x="${padding + ruleWidth + Math.round(padding * 0.5)}" y="${baseline}"
        ...
        fill="${GOLD}" fill-opacity="0.94">${escapeXml(WATERMARK_WORDMARK)}</text>
  <text x="${width - padding}" y="${baseline}" text-anchor="end"
        ...
        fill="#f2f0ec" fill-opacity="0.78">${escapeXml(WATERMARK_DOMAIN)}</text>
</svg>`,
    "utf8",
  );
}
